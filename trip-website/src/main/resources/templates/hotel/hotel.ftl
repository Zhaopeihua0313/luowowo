...
<div class="hotel-main">
    ...
    <form class="form-hotel" action="/hotel/query" method="get" id="editForm">
        <div class="clearfix date-warp">
            <div class="add-travle">
                <input type="text" placeholder="出行目的地" id="_j_search_input" autocomplete="off" name="keyword">
                <div class="not-cont" id="_j_search_shortcut_mdds" style="display: none;">
                    <ul id="message">
                        <li class="clearfix">
                            ...
                            <p>
                            <#list inland as d >
                                <a href="hotel/query?destId=${d.id}" data-id="${d.id}" data-name="${d.name}">${d.name}</a>
                            </#list>
                        </li>
                        <li class="clearfix">
                            </p>
            ...
            <div class="btn-search" id="_A_search_btn">
                <a role="button" class="btn-sss"><i class=""></i></a>
            </div>
        </div>
    </form>
    ...
    <div class="h-theme" id="_j_features">
        ...
        <div class="tab-theme">
            <div class="themeList clearfix _j_tab_trigger" id="themeList">
            <#list hotelTags as theme>
                <a href="javascript:void(0);" id="type-theme${theme_index}"
                   onclick="t${theme_index}();" data-id="${theme.id}">${theme.name}</a>
                <script>
                    t${theme_index} = function () {
                        ...
                        $(this).addClass("on");
                        // 发送请求
                        $.get("/hotel/listByTag", {tagId: id}, function (data) {
                            $("#clearfixMessage1").html(data);
                        })
                    }
                </script>
            </#list>
            </div>
            <div class="_j_tab_content" id="clearfixMessage1">
            </div>
        </div>
    </div>
        ...
        <div class="tab-fav">
            ...
            <#list hotelCity as city>
                <a href="javascript:void(0);" name="c" id="lucky${city_index}" data-id="${city.id}"
                    onclick="test${city.id}();">${city.name}</a>
                <script>
                    test${city.id} = function () {
                        var id = $("#lucky${city_index}").data('id');
                        if (!id) { // 防止没有获取到值
                            return;
                        }
                        $(this).addClass("on");
                        // 发送请求
                        $.get("/hotel/listByCity", {destId: id}, function (data) {
                            $("#clearfixMessage2").html(data);
                        })
                    }
                </script>
            </#list>
            </div>
            <div class="tab-con _j_tab_content" id="clearfixMessage2">
            </div>
        </div>
    </div>
</div>
